...
[English](./README.md) | [简体中文](./README.zh-CN.md) | 繁體中文（台灣） | [繁體中文（香港）](./README.zh-HK.md)
...
| 產品 | 功能 | 開始使用 |
| --- | --- | --- |
| **Freebuff Desktop** | 在本機並行執行多個智能體 | [下載 macOS、Windows 或 Linux 版](https://freebuff.com/desktop) |
| **Freebuff CLI** | 從終端機編寫程式碼 | [安裝 CLI](https://freebuff.com/cli) |
| **Freebuff Web** | 建置並發布全端應用程式 | [建置應用程式](https://freebuff.com/web) |
...
| **Muse Spark 1.3** | 完整存取 | Meta 的智能體編碼模型；100 萬上下文。有速率限制且由所有使用者共享，繁忙時會排隊，並改由 DeepSeek V4 Flash 回答，不會讓你久候 |
...
可用性與限制取決於你的存取等級、所用產品及目前容量。Freebuff Desktop 也可以使用你現有的提供者帳號來執行本機安裝的 Claude Code 和 Codex 智能體；這些連接的模型與 Freebuff 內建的模型目錄各自獨立。
...
- **本機並行工作** —— Desktop 將並行的智能體隔離在各自的工作區中。
- **託管環境** —— Web 和 Cloud 提供沙箱、預覽、終端機和部署工作流程。

## 免費存取

Freebuff 在所有國家與地區皆可使用。受支援的地區可獲得完整存取；其他地區與 VPN 使用者可獲得對 DeepSeek V4 Flash 07/31、GLM 5.3 Flash、MiMo 2.5 和 Solar Pro 4 的受限存取。使用 Freebucks 的帳號按顯示的模型價格與餘額計費。在舊版會話系統中，受限存取每天包含六個一小時的會話，最多可累積至七個；GLM 則改用賺取的獎勵會話。
...
**我的資料會如何使用和儲存？** 我們會使用提示詞、訊息、智能體軌跡、程式碼、檔案與儲存庫資料來提供 Freebuff 服務。我們可能分析提示詞和訊息以個人化廣告。我們不會將單獨上傳的檔案或已連接的儲存庫提供給廣告提供者。受限制的合作夥伴可能評估已連接的 Cloud 儲存庫或標示為「May use data for AI training」（資料可能用於 AI 訓練）的模型所使用的程式碼，但除此之外不得使用、廣泛分享或以其進行訓練。關於保留期限、資格與廣告選項，請參閱隱私權政策。

完整詳情請參閱[隱私權政策](https://freebuff.com/privacy-policy)。
...
本機開發需要 Docker 和已設定的 `.env.local`；啟動服務之前請先參閱[貢獻指南](./CONTRIBUTING.md)。
...
- [隱私權政策](https://freebuff.com/privacy-policy)
- [授權條款](./LICENSE)