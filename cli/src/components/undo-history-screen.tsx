...
export const UndoHistoryScreen: React.FC<UndoHistoryScreenProps> = ({
  ...
  // What the action actually touches on disk — not just the selected turn.
  // An undo reverts the selected turn and every newer one; a redo brings back
  // the turns its undo had reverted. Listing only the focused entry's files
  // understated the blast radius the notice below it warns about.
  const affectedFiles = useMemo(() => {
    if (!focusedRecord) return []
    return mode === 'undo'
      ? filesTakenBack(entries, focusedRecord.id)
      : filesRestored(focusedRecord)
  }, [mode, entries, focusedRecord])

  const visibleFiles = affectedFiles.slice(0, LAYOUT.MAX_VISIBLE_FILES)
  const hiddenFiles = affectedFiles.length - visibleFiles.length

  // The journal takes the selected entry *and everything newer* with it (a
  // redo drops the newer redo entries the same way). Say so while the row is
  // merely focused, not in the confirmation afterwards: by then the work is
  // already gone.
  const cascadeCount = focusedRecord
    ? newerTurnCount(entries, focusedRecord.id)
    : 0
  const cascadeNotice =
    cascadeCount === 0
      ? null
      : mode === 'undo'
        ? `⚠ Also reverts ${cascadeCount} newer ${cascadeCount === 1 ? 'change' : 'changes'}`
        : `⚠ Also discards ${cascadeCount} newer ${cascadeCount === 1 ? 'redo' : 'redos'}`
      ...
      >
        ...
        >
          <text
            style={{ fg: theme.foreground, attributes: TextAttributes.BOLD }}
          >
            {focusedRecord ? `Files (${affectedFiles.length})` : 'Files'}
          </text>
