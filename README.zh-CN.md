...
大多数模型使用你的常规每日会话，而不再各自设限。GLM 5.3 Flash、DeepSeek V4.1 Flash、MiMo 2.6 Flash 和 Solar Pro 4 在完整访问下保持无限使用，完全不消耗会话。模型仍可能由量化（Q8_0）版本提供服务。
...
Freebuff 在所有国家和地区均可使用。受支持的地区提供完整访问；其他地区以及使用 VPN 的用户获得受限访问，包括 DeepSeek V4.1 Flash、GLM 5.3 Flash、MiMo 2.6 Flash 和 Solar Pro 4。使用 Freebucks 的账户按显示的模型价格和余额计费。在旧版会话系统中，受限访问每天提供六个一小时会话，最多可赚取七个；GLM 则使用赚取的奖励会话。
